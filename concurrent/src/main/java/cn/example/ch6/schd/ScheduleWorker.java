...
public class ScheduleWorker implements Runnable{
    ...
    public final static int ProcessException=1;//抛出异常会捕捉的任务类型
    ...
    public void run() {
        if (taskType==HasException){
            ...
            try {
                System.out.println("ProcessException..."+formater.format(new
                        Date()));
                throw new RuntimeException("ProcessException");
            }catch (RuntimeException e){
                System.out.println("ProcessException cached ,will next task run?");
            }
        }else {
