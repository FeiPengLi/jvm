...
public class PendingJobPool {
    ...
    //构造私有，以单例模式启动,延迟占位类模式
    private PendingJobPool(){}

    private static class JobPoolHolder{
        public static PendingJobPool pool=new PendingJobPool();
    }
    public static PendingJobPool getInstance(){return JobPoolHolder.pool;}
    ...
    private static class PendingTask<T,R>implements Runnable{
        ...
        public void run() {
            R r=null;
            ITtaskProcessor<T,R> taskProcesser= (ITtaskProcessor<T, R>)
                    jobInfo.getTaskprocessor();
    ...
    public <R>  void registerJob(String jobName,int jobLength,
                                 ITtaskProcessor<?,?>taskProcesser,long expireTime){
        JobInfo<R> jobInfo=
                new JobInfo<>(jobName,jobLength,taskProcesser,expireTime);
